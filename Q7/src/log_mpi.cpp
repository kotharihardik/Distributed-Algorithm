...
int main(int argc, char **argv)
{
    ...
    // Move to the start of a complete line
    if (low > dataStart)
    {
    ...
    double respSum = 0, respMin = DBL_MAX, respMax = -DBL_MAX;

    vector<long long> serverCount(S, 0);
    vector<double> serverResp(S, 0);

    vector<long long> rawEp, rawBytes, rawIv;

    long long localEpMax = -1, localIvMin = LLONG_MAX, localIvMax = LLONG_MIN;
    ...
    MPI_Allreduce(&localEpMax, &epMax, 1, MPI_LONG_LONG, MPI_MAX, MPI_COMM_WORLD);
    MPI_Allreduce(&localIvMin, &ivMin, 1, MPI_LONG_LONG, MPI_MIN, MPI_COMM_WORLD);
    MPI_Allreduce(&localIvMax, &ivMax, 1, MPI_LONG_LONG, MPI_MAX, MPI_COMM_WORLD);

    long long numEp = epMax >= 0 ? epMax + 1 : 0;
    long long numIv = ivMax >= ivMin ? ivMax - ivMin + 1 : 0;

    vector<long long> epCount(numEp, 0), epBytes(numEp, 0), ivCount(numIv, 0);
    ...
    for (size_t i = 0; i < rawEp.size(); i++)
    {
        ...
        if (pos >= 0 && pos < numIv) ivCount[pos]++;
    }

    double countTime = MPI_Wtime();

    long long local[8] = {total, ok, failed, s2xx, s3xx, s4xx, s5xx, totalBytes};
    long long global[8] = {0};

    MPI_Reduce(local, global, 8, MPI_LONG_LONG, MPI_SUM, 0, MPI_COMM_WORLD);

    double globalRespSum = 0, globalRespMin = 0, globalRespMax = 0;

    MPI_Reduce(&respSum, &globalRespSum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(&respMin, &globalRespMin, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
    MPI_Reduce(&respMax, &globalRespMax, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

    vector<long long> globalServerCount(rank == 0 ? S : 0, 0);
    vector<double> globalServerResp(rank == 0 ? S : 0, 0);
    ...
    if (rank == 0)
    {
        ...
        for (long long i = 0; i < S; i++)
        {
            if (globalServerCount[i] > 0)
                servers.push_back({i, globalServerCount[i], globalServerResp[i], 0});
        }
        ...
        for (long long i = 0; i < numEp; i++)
        {
            if (globalEpCount[i] > 0)
                endpoints.push_back({i, globalEpCount[i], 0, globalEpBytes[i]});
        }
        ...
        for (size_t i = 0; i < servers.size() && (long long)i < K; i++)
        {
            printf("%lld %lld %.6f\n", servers[i].id, servers[i].count,
                   servers[i].count ? servers[i].resp_sum / servers[i].count : 0.0);
        }

        printf("TOP_ENDPOINTS\n");

        for (size_t i = 0; i < endpoints.size() && (long long)i < K; i++)
        {
            printf("%lld %lld %lld\n",
                   endpoints[i].id, endpoints[i].count, endpoints[i].bytes);
        }
    }
    ...
    {
        double times[4] = {
            readTime - start, countTime - readTime,
            endTime - countTime, endTime - start
        };

        double worst[4];

        MPI_Reduce(times, worst, 4, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
